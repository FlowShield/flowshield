...
# CloudHole-****FVM**** build notes  [(whitepaper)](https://www.cloudhole.xyz/cloudhole_whitepaper/)

- [CloudHole-****FVM**** build notes  (whitepaper)](#cloudhole-fvm-build-notes--whitepaper)
  ...
  - [How is **FVM**, Filecoin, IPFS technology used in this project](#how-is-fvm-filecoin-ipfs-technology-used-in-this-project)
    - [CloudHole-Fullnode - build with IPFS|libp2p](#cloudhole-fullnode---build-with-ipfslibp2p)
    - [CloudHole-Provider - build with IPFS|libp2p](#cloudhole-provider---build-with-ipfslibp2p)
  ...
  - [**FVM**'s current work contents](#fvms-current-work-contents)
    - [CloudHole-contracts **FVM** adaptation process record](#cloudhole-contracts-fvm-adaptation-process-record)
    - [Deploy the contract on `wallaby`](#deploy-the-contract-on-wallaby)
    - [CloudHole-provider **FVM** adaptation process record](#cloudhole-provider-fvm-adaptation-process-record)


## Project Description

CloudHole aims to build a decentralized web3 privacy data retrieval security network system around the world, and help users recapture privacy security information eroded by giants under web2, so that the current global hot zero-trust security network technology combined with web3 can better help users master their own security privacy data and give users a good experience of privacy data retrieval security network products.

## Project **FVM** version links
 
Link to Github repo: https://github.com/cloudhole-project/CloudHole/tree/fevm-testnet

**FVM** testnet website: https://fvm-testnet.CloudHole.xyz/

## How is **FVM**, Filecoin, IPFS technology used in this project

### CloudHole-Fullnode - build with IPFS|libp2p

All nodes are CloudHole networks, which provide DAO tools for institutions\organizations\individuals, mainly to satisfy users’ privatization and customize their own network privatization attributes.
...
### CloudHole-Provider - build with IPFS|libp2p

Provider, as a secure network tunnel provider for decentralized private data retrieval, provides decentralized network access services for CloudHole network users. The Provider decentralized networking through peer-to-peer discovery and routing through libp2p, and combined with the publish and subscribe function of P2P to achieve data synchronization among multiple nodes.
...
Decentralized PKI CA center provides communication authentication function for point-to-point communication between client and miner nodes in CloudHole network. DeCA can perform all the key functions of X.509 PKI standard, that is, register, confirm, revoke and verify mTLS certificates.
...
1. Adapting CloudHole fullnode, network provider, smart contracts on fevm test network.
   
2. Deploy CloudHole-FVM's exclusive website, and let CloudHole platform run on FVM's test network.
   
3. Upgrade CloudHole provider and fullnode pledge process to adapt to **FVM** test network.


### CloudHole-contracts **FVM** adaptation process record
### Deploy the contract on `wallaby`

Our contract warehouse address is [https://github.com/cloudhole-project/CloudHole/tree/fevm-testnet/contract](https://github.com/cloudhole-project/CloudHole/tree/fevm-testnet/contract)
...
yarn run v1.22.17
$ /Users/chengqiang/icode/github/CloudHole/CloudHole/contract/node_modules/.bin/hardhat get-address
...
yarn run v1.22.17
$ /Users/chengqiang/icode/github/CloudHole/CloudHole/contract/node_modules/.bin/hardhat deploy
You have both ethereum-waffle and @nomicfoundation/hardhat-chai-matchers installed. They don't work correctly together, so please make sure you only use one.
...
deploying "CloudHoleDao" (tx: 0xdfa457bbb69b7d317236d6f6665ca34bfba7d971d33da34c52b6c6ba6a054864)...: deployed at 0x7D9208Aa55833A9F5641cD2d7477733D819435eC with 70991164 gas
✨  Done in 76.95s.
```
...
### CloudHole-provider **FVM** adaptation process record
